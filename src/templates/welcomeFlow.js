...
async function handleAppointmentType (ctx, ctxFn, bodyText) {
  ...
  ) { appointmentType = 'virtual' } else if (response.includes('2') || response.includes('presencial')) { appointmentType = 'inPerson' }
  if (appointmentType) {
    await ctxFn.state.update({
      appointmentType,
...
async function handleNormalConversation (
  ...
  try {
    ...
    if (previousInteractions && previousInteractions.length > 1) { isFirstInteraction = false }
  } catch (error) {
    logger.error(
  ...
  if (isFirstInteraction) {
    ...
    if (appointmentsEnabled || hasDocuments || hasImages) {
      ...
      if (appointmentsEnabled) {
        let citasMessage = '\n•⁠  ⁠Agendar citas'
        if (virtualEnabled && inPersonEnabled) { citasMessage += ' (virtuales o presenciales)' } else if (virtualEnabled) citasMessage += ' virtuales'
